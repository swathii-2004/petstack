...
export default function PendingApprovalPage() {
  ...
  // Poll backend every 30s to check if status changed
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        } else if (backendUser.status === "rejected") {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-lg text-center">
        {/* Animated Clock */}
        <div className="w-24 h-24 bg-sl-indigo/10 rounded-full flex items-center justify-center mx-auto mb-6 relative">
          <div className="absolute inset-0 rounded-full border-4 border-sl-indigo/20 animate-ping" />
          <Clock className="text-sl-indigo" size={40} />
        </div>
        ...
        <p className="text-sl-text-mid text-[14.5px] mb-8 leading-relaxed max-w-md mx-auto">
          Your seller application has been submitted successfully. Our team reviews all applications within <strong>24–48 hours</strong>. You'll receive an email once a decision is made.
        </p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-6 mb-6 text-left">
          ...
          {user && (
            <div className="space-y-1.5 text-[13px]">
              ...
              {(user as any).business_name && <div className="flex justify-between"><span className="text-sl-text-mid">Business</span><span className="font-semibold text-sl-text-dark">{(user as any).business_name}</span></div>}
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={checkNow}
            disabled={checking}
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-sl-indigo text-white rounded-xl font-semibold text-[14px] hover:bg-sl-indigo-dk transition-colors disabled:opacity-60"
          >
